...
def _open_readonly(path: str | os.PathLike[str], *, dir_fd: int | None = None) -> int:
    """Open safely, retaining O_NOATIME when ownership permits it."""
    try:
        return os.open(path, _READ_FLAGS, dir_fd=dir_fd)
    except OSError as error:
        noatime = getattr(os, "O_NOATIME", 0)
        if error.errno != errno.EPERM or not noatime:
            raise
        return os.open(path, _READ_FLAGS & ~noatime, dir_fd=dir_fd)
